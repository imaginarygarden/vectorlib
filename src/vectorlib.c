...
vector_result_t vector_init(vector_t *restrict vector, const size_t data_size) {
    ...
    if (vector == NULL) {
        DEBUG_ERROR("[VECTORLIB] Failed to initialize vector %p due to invalid valid pointer\n", vector);
...
vector_result_t vector_set(vector_t *restrict vector, const size_t index, const void *restrict value) {
    ...
    if (index >= vector->size) {
        DEBUG_ERROR("[VECTORLIB] Failed to set value in vector %p outside of bounds\n", vector);
